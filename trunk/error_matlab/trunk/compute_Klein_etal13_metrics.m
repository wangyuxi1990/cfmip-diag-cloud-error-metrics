...
% This script written by Mark Zelinka (zelinka1@llnl.gov) on 7 October 2013
...
x=[lon;lon(1)+360];
avg_albcs2 = interp3(lat,1:12,x,cat_albcs,kern_lat,1:12,kern_lon');  
avg_clisccp_mod=nan*ones(12,7,7,90,144);
for T=1:7
    for P=1:7
        cat_clisccp=cat(3,squeeze(avg_clisccp(:,T,P,:,:)),squeeze(avg_clisccp(:,T,P,:,1)));    
        avg_clisccp_mod(:,T,P,:,:) = interp3(lat,1:12,x,cat_clisccp,kern_lat,1:12,kern_lon');
    end
end
clear avg_clisccp avg_albcs
...
ctot=squeeze(nansum(nansum(anomobs(:,:,:,pm60,:),2),3)); % size(12,Lreg,144)
...
for dd=1:2 % Loop through possible differences       
    ...
    ctot=squeeze(nansum(nansum(anompctau(:,:,:,pm60,:),2),3)); % size(12,Lreg,144)
...
SWkernel(:,1:cutoff,:,:,:)=[];
LWkernel(:,1:cutoff,:,:,:)=[];
[a,b,c,d,e,f]=size(avg_clisccp_mod);
SWkernelmod=NaN*ones(12,b,c,d,e);
for mm=1:12
    for i=1:90
        alon=squeeze(avg_albcs2(mm,i,:)); 
        if numel(alon(isnan(alon)))>0; continue; end
        SWkernelmod(mm,:,:,i,:)=interp3(1:7,1:b,albcs_midpt,squeeze(SWkernel(mm,:,:,i,:)),1:7,1:b,alon);
    end
end
